...
class XiaohongshuContentGenerator:
    ...
    def build_xiaohongshu_prompt(news_list: List[Dict]) -> str:
        ...
        prompt = f"""你是一个擅长写小红书内容的资深文案，获得过金瞳奖。请按照以下部分结构，为我生成一篇完整的内容。
...
【第三部分：正文】
总字数：600-800字左右，但这是总参考，不是各部分硬指标，少量低饱和度 emoji 分段点缀。
第一层：事件简述+引入观点 

要求：有逻辑的概述新闻事实。重点是让读者了解新闻始末，快速地从事实过渡到你想探讨的核心话题，用一个问句或一个独特的观察视角来开头。

第二层：深度拆解 

要求：这是体现深度的部分。要求找出1-2个最能切中目标受众（高知女性）痛点的角度进行剖析。这个角度必须从给定的方向（如引发讨论、激发共鸣、情绪价值、平台算法、消费陷阱等）中选取。

写法：每个角度用1-2个小标题或分段来写，每段内容要“由表及里”，先描述现象，再揭示本质。内容要有“信息增量”，即读者不知道或没想过的点。

第三层：共鸣与代入 

要求：这是“神转折”或“情感升华”的部分。用一个具体的、有画面感的场景（比如“试想一下，你下班回家刷到这条新闻……”）来引发读者共鸣。将宏大叙事，落到每个女生具体的生活和情感体验上。语气要松弛，可以有适当的自嘲。

第四层：观点升华与实用建议 

要求：输出你的核心观点，要犀利但不偏激。为读者提供1-2个具有实操性的“思考工具”或“行动建议”。这是体现你“互联网闺蜜”价值的地方，要让读者觉得“学到了”或“被点醒了”。语气是“提醒”而非“说教”。
